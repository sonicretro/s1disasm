...
local function CreateMD5Object()
	...
	local function PushData(data)
		total_bits = total_bits + 16 * 4 * 8

		-- Unpack the bytes from a string to a table.
		ProcessBlock({string.unpack("\z
	...
	local function PushFinalData(data, bits)
		...
		for i = 0, 3 do
			string_table[1 + 4 * 0 + i] = string.format("%02X", (A >> (8 * i)) & 0xFF)
			string_table[1 + 4 * 1 + i] = string.format("%02X", (B >> (8 * i)) & 0xFF)
			string_table[1 + 4 * 2 + i] = string.format("%02X", (C >> (8 * i)) & 0xFF)
			string_table[1 + 4 * 3 + i] = string.format("%02X", (D >> (8 * i)) & 0xFF)
		end
